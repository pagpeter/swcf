...
use swc_core::ecma::ast::{
    op, ArrayLit, AssignExpr, BinExpr, BinaryOp, CallExpr, Callee, Decl, Expr, ExprStmt, FnDecl,
    Id, Ident, Lit, ModuleItem, Number, Program, Stmt, Str, VarDeclarator,
};
...
impl VisitMut for Visitor {
    fn visit_mut_program(&mut self, program: &mut Program) {
        ...
        let mut index_fn_visitor = FindIndexFunctionVisitor::new(get_obf_strings_fn_id.clone());
        ...
        let mut expr_visitor = FindObfExpression::new(get_obf_strings_fn_id.clone());
        program.visit_mut_children_with(&mut expr_visitor);
        // The original expression
        let original_expr = try_unwrap!(expr_visitor.expr, "Array compute function not found");
        ...
        loop {
            ...
            let mut expr_evaluator = ExprVisitor::new(index_data.clone(), &obfuscated_strings);
...
impl VisitMut for FindIndexFunctionVisitor {
    fn visit_mut_fn_decl(&mut self, fn_decl: &mut FnDecl) {
        ...
        if self.is_inside_correct_fn
            && self.fn_id.is_none()
            && self.get_obfuscated_strings_fn != fn_decl.ident.to_id()
        {
            self.fn_id = Some(fn_decl.ident.to_id());
            fn_decl.take();
        }
                ...
                if let Expr::Call(get_index_call) = &*argument.expr {
                    ...
                    if let Some(offset_arg) = get_index_call.args.get(0) {
                        if let Expr::Lit(Lit::Num(offset)) = &*offset_arg.expr {
                            // NaN as a node
                            let nan = Expr::Ident(Ident::new(JsWord::from("NaN"), Span::default()));

                            // Replace node
                            *expr = match get_index(
                                offset.value as u32,
                                self.index_data.offset as u32,
                                self.index_data.op,
                            ) {
                                Some(index) => match self.obfuscated_strings.get(index as usize) {
        ...
        if let Expr::Call(call_expr) = expr {
            ...
            if let Callee::Expr(callee_expr) = &call_expr.callee {
                if let Expr::Ident(id) = &**callee_expr {
                    ...
                    if let Some(real_index) = get_index(
                        index as u32,
                        self.index_data.offset as u32,
                        self.index_data.op,
                    ) {
