...
use swc_core::ecma::ast::{
    Decl, Expr, FnDecl, Id, Ident, ModuleItem, Pat, Program, Stmt, VarDeclarator,
};
            ...
            if let Some(expr) = &declarator.init {
                if let Expr::Ident(fn_id) = &**expr {
                    ...
                    if self.functions.contains(&fn_id.to_id()) {
                         ...
                         */
                        let replacement_sym =
                            if let Some(highest_ctx) = self.identifiers.get(&fn_id.sym) {
                                if fn_id.span.ctxt < *highest_ctx {
                                    ...
                                    self.function_replacements
                                        .insert(fn_id.to_id(), replacement.clone());
